...
function createHistoryPatientTable(): void {
    // Create new window
    ...
    historyPatientTable.on('close', (): void => {
                ...
                click() {
                    dialog.showMessageBox({
                        ...
                        if (index.response == 1) {
                            ...
                            if (historyPatientTable != null) historyPatientTable.close();
                            mainWindow.reload();
                        }
                    })
                }
...
ipcMain.on("patientlist:register", (e, item: any[]) => { // 表单提交
...
/** 医生监听
 */
function updateHistoryWindow(): void { 
...
ipcMain.on("doctor:next_patient", (e, department, doctor) => {
    ...
    // 更新医生状态
    let doctorStatus: boolean = patientManageSystem.getDepartment(department).getDoctor(doctor).getStatus();
...
ipcMain.on("doctor:done", (e, department, doctor) => {
    ...
    if (leavePatient.bTag == true) {
        if (leavePatient.bTreatmentTime == "") {
            ...
            // 更新 B 超室状态
            for (let i = 1; i <= 3; i++) {
                let doctorStatus: boolean = patientManageSystem.getDepartment(16).getDoctor(i).getStatus();
            ...
            // 更新原科室状态 科室列表不用更新
            let doctorStatus: boolean = patientManageSystem.getDepartment(retDep).getDoctor(retDoc).getStatus();
...
ipcMain.on("doctor:historyPatientList", (e, department, doctor) => {
    ...
    if (historyPatientTable == null) createHistoryPatientTable(); // 新建病人列表窗口
    else updateHistoryWindow(); // 窗口已弹出，则只更新内容
});
