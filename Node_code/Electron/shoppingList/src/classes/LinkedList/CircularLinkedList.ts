...
export class CircularLinkedList<T> extends LinkedList<T> {
    ...
    insert(element: T, ith: number = this.cnt): number {
        ...
        if (ith == 0) { // insert front
            node.next = this.head;
            this.head = node;
            if (this.tail == null) this.tail = node, this.tail.next = this.head;
            this.tail.next = node;
        } else {
            while (index != this.cnt) {
                if ((++index) == ith) {
                    ...
                    cur.next = node;
                    node.next = tmp;
                    if (cur == this.tail) this.tail = cur.next;
                    break;
                }
                cur = cur.next;
            }
    ...
    deleteElement(element: T): number {
        ...
        while (index != this.cnt) {
            ...
            if (cur.element === element) {
                if (prev == this.tail) this.head = cur.next, prev.next = this.head;
                else {
    ...
    deleteIth(ith: number): number {
        ...
        while (cur != null) {
            if ((++index) == ith) {
                if (ith == 1) this.head = cur.next, this.tail.next = this.head;
                else {
                    if (cur == this.tail) this.tail = prev;
                    prev.next = cur.next;
                }
