...
from .validity import validity_check
...
class ECOTracker:
    ...
    def update(self, frame, train=True, vis=False):
        ...
        for _ in range(self.config.refinement_iterations):
            ...
            if not np.allclose(old_pos, pos):
                ...
                if vis:
                    ...
                    self.std_score = validity_check(self.score, K=200)
        ...
        if not scores_fs is None:
            score = xp.fft.fftshift(sample_fs(scores_fs[:,:,scale_idx],
                                tuple((10*self._output_sz).astype(np.uint32))))
            std_score = validity_check(score, K=200)
            # std_score = 1.0

            # model udpate step
            if not sample_scale is None and not scale_idx is None and not xtf_proj is None:
                xlf_proj = None
