...
inline void build(int L, int R, int root) {
	if (L == R) return;
	int mid = (L + R) >> 1;
	build(L, mid, newNode(T[root].lson)), build(mid + 1, R, newNode(T[root].rson));
}
inline void modify(int pre, int cur, int L, int R, int pos) {
	...
	int mid = (L + R) >> 1;
	if (pos <= mid) modify(T[pre].lson, newNode(T[cur].lson), L, mid, pos);
	else modify(T[pre].rson, newNode(T[cur].rson), mid + 1, R, pos);
...
inline int query(int pre, int cur, int L, int R, int times) {
	...
	if (cntL * 2 > times) 
		if (ans = query(T[pre].lson, T[cur].lson, L, mid, times)) return ans; //结果不为-1才返回
	if (cntR * 2 > times) 
		if (ans = query(T[pre].rson, T[cur].rson, mid + 1, R, times)) return ans; //结果不为-1才返回
	return 0;
}
int main(){
	n = read(), m = read();
