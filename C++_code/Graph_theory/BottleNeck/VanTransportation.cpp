...
void dfs(int cur, int fa, int w) {
	...
	for (int i = 1; i < 15; i++) //LCA预处理
		f[cur][i] = f[f[cur][i - 1]][i - 1],
		val[cur][i] = min(val[cur][i - 1], val[f[cur][i - 1]][i - 1]);
...
inline int LCA(int u, int v, int Max) {
	...
	for (int i = 14; i >= 0; i--)
		if (f[u][i] != f[v][i]) {
			Max = min(Max, min(val[u][i], val[v][i]));
			u = f[u][i], v = f[v][i];
		}
	return min(Max, min(val[u][0], val[v][0]));
}
int main(){
	Init();
	kruskal();
