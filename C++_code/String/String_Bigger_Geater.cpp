...
inline int get_closest_p(int pos, string s){
    ...
    for(int i=pos+1;i<s.size();i++)
        if(s[i]-s[pos]<minval&&s[i]-s[pos]>0) minval=s[i]-s[pos], p=i;
...
inline void swap_string(string &s){
    ...
    if(!Swap){cout<<"no answer"<<endl;return;}
    sort(s.begin()+Index+1,s.end());//sort the rest of string to make it the smallest
    cout<<s<<endl;
    return;
}
int main(){
    ...
    cin>>T;
    cin.ignore();
    while(T--){ string s; getline(cin,s); swap_string(s); }//initalize
    return 0;
}
/*
#include <bits/stdc++.h>
...
int main(){
    ...
    while(test--) {
        string inp;
        cin >> inp;
        if (next_permutation(inp.begin(), inp.end()) == false ) { prev_permutation 前一个全排列
            cout << "no answer" << endl;
        }
        else {
            cout << inp << endl;
